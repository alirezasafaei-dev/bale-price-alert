...
def sync_files_with_sftp(ssh):
    ...
    exclude_dirs = {
        '.git', '__pycache__', '.venv', 'node_modules', '.next', 'dist',
...
def main():
    ...
    try:
        ...
        ssh_exec_with_check(
            ssh,
            f"cd {APP_DIR}/mini-app && npm install",
            "Installing Node dependencies"
        )
        ssh_exec_with_check(
            ssh,
            f"cd {APP_DIR}/mini-app && npm run build",
            "Building mini-app"
        )
        ...
        ssh_exec_with_check(
            ssh,
            f"cd {APP_DIR} && alembic upgrade head",
        ...
        ssh_exec_with_check(
            ssh,
            f"cd {APP_DIR} && pm2 restart novax-api --update-env",
            "Restarting novax-api"
        )
        ssh_exec_with_check(
            ssh,
            f"cd {APP_DIR} && pm2 restart novax-worker --update-env",
            "Restarting novax-worker"
        )
        ssh_exec_with_check(
            ssh,
        ...
        ssh_exec_with_check(
            ssh,
            "curl -s http://127.0.0.1:8001/api/v1/prices/latest",
            "Prices API Check"
        )
